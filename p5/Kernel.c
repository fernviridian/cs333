code Kernel
  ...
  --
  -- CODENAME: Harry Porter and the Deathly Deadlock
  --
  -- REFERENCES:
  -- https://github.com/lewisric/Frogs-N-Feels/blob/master/Project4/Kernel.c
  -- https://github.com/ladinu/cs333/blob/p5/p4/submit/Kernel.c
  -- Talking with Tyler Sorg about Hoare Semantics
  -- https://github.com/ladinu/cs333/blob/p5/p5/Kernel.c
  -- https://github.com/lewisric/Frogs-N-Feels/blob/master/Project5/Kernel.c
...
---------------------------- HOARE CONDITION --------------------------
  ...
  behavior HoareCondition
    ...
    method Init()
        waitingThreads = new List [Thread]
        -- no need to initialize mutex since it is passed into methods as a ptr to a mutex
      endMethod
    ...
    method Signal(mutex: ptr to Mutex)
      ...
      -- copied again from Condition, with a few minor hoare semantic changes
         var
          oldIntStat: int
          t: ptr to Thread
        if ! mutex.IsHeldByCurrentThread ()
        ...
        oldIntStat = SetInterruptsTo (DISABLED)
        t = waitingThreads.Remove ()
        mutex.heldBy = t -- ADDED: handoff to next thread in list per Hoare semantics
        if t
          t.status = READY
          readyList.AddToEnd (t)
        endIf
        mutex.Lock() -- ADDED: give new thread we just handed to the lock
        oldIntStat = SetInterruptsTo (oldIntStat)
      endMethod
    ...
    method Wait(mutex: ptr to Mutex)
        -- "Third verse, same as the....you get the idea"
        var
          oldIntStat: int
        if ! mutex.IsHeldByCurrentThread ()
          FatalError ("Attempt to wait on condition when mutex is not held")
        ...
        currentThread.Sleep ()
        -- mutex.Lock ()
        -- we dont need to lock the lock and wait for another thread to acquire since we "handoff" the lock to 
        -- the other waiting thread directly
        oldIntStat = SetInterruptsTo (oldIntStat)
      endMethod
 endBehavior
  ...
  behavior ThreadManager
      ...
      method Init ()
        ...
        -- 
          ...
          -- set threads to UNUSED on freeList
          for i = 0 to MAX_NUMBER_OF_PROCESSES - 1
            threadTable[i].Init("thread")
            threadTable[i].status = UNUSED
            -- thread is free so we should add to freeList
            freeList.AddToEnd(&threadTable[i]) -- have to remember to pass address, not actual object to list
          endFor
        endMethod
      ...
      method GetANewThread () returns ptr to Thread
          ...
          -- 
          -- if there is a a free thread slot in the freeList we allocate that
          -- and hand it back to the calling routing that wanted a thread
          var
            newThread: ptr to Thread

          newThread = null -- not pointing quite yet

          threadManagerLock.Lock()

          -- MESA-style semantic sanity check
          -- while freeList.IsEmpty()  -- if the list is full, and we cant allocate a thread, we should wait
          ...
          -- now we know we hold the lock and a thread position is available
          newThread = freeList.Remove() -- get pointer to thread
      ...
      method FreeThread (th: ptr to Thread)
          ...
          -- to the FREE list.
          -- 
          -- We have a thread but dont need it anymore, so give it up and 
          -- hand it off to someone else

          threadManagerLock.Lock()

          -- thread is now unused so we can add its thread slot back to the free list of threads
          th.status = UNUSED
          freeList.AddToEnd(th)
          -- we should signal that a thread is now free or else os waiting for free thread condition will starve
  ...
  behavior ProcessManager
      ...
      method Init ()
        ...
        --
          ...
          nextPid = 0 -- not sure if need to init here or somewhere else?
          ...
          -- create freeList and initialize all the ProcessControlBlocks
          ...
          -- add all PCBs to freeList
          for i = 0 to MAX_NUMBER_OF_PROCESSES - 1
            processTable[i].Init()
            freeList.AddToEnd(&processTable[i])
            processTable[i].status = FREE  -- we should set status to FREE since we added it to the freeList
          endFor
        endMethod
      ...
      method GetANewProcess () returns ptr to ProcessControlBlock
          ...
          -- until one is available.
          --
          -- a LOT of this is coppied from GetANewThread.
          var
            newProcess: ptr to ProcessControlBlock

          newProcess = null -- not pointing quite yet
          ...
          -- increment the process id, pid
          nextPid = nextPid + 1

          -- now we know we hold the lock and a thread position is available
          newProcess = freeList.Remove() -- get pointer to thread
      ...
      method FreeProcess (p: ptr to ProcessControlBlock)
        ...
        -- to the FREE list.
        --
        -- most code reused from FreeThread. 
          processManagerLock.Lock()
          p.status = FREE
          freeList.AddToEnd(p)
...
-----------------------------  FrameManager  ---------------------------------
  ...
  behavior FrameManager
      ...
      method GetNewFrames (aPageTable: ptr to AddrSpace, numFramesNeeded: int)
          -- From http://web.cecs.pdx.edu/~harry/Blitz/OSProject/p4/proj4.htm
          ...
          for i = 0 to numFramesNeeded - 1
            -- get which frame is free using BitMap
            freeFrame = framesInUse.FindZeroAndSet() -- frames are free if zero bit is set, so use this method to find next free
            -- now that we have the frame number, we need to get the address of the frame relative to offsets
            frameAddr = PHYSICAL_ADDRESS_OF_FIRST_PAGE_FRAME + (freeFrame * PAGE_SIZE)
            -- example (obviously we are dealing with much larger sizes of memory)
            -- we start at say address 100 for first page frame.
            -- if PAGE_SIZE was 8, then in order to find page 4
            -- we would need to multiple PAGE_SIZE by page number and then offset that
            -- with the first page frame base, which would get us address 132 for the frame.
            aPageTable.SetFrameAddr(i, frameAddr)
          endFor
          ...
          frameManagerLock.Unlock() -- I get an unlock exception if this is left here. relevant: https://i.imgur.com/x0ml8.png
        endMethod

      ----------  FrameManager . ReturnAllFrames  ----------

      method ReturnAllFrames (aPageTable: ptr to AddrSpace)
          -- TODO should revisit to make sure there is no starvation conditions in Frame logic
          --
          var
            i: int
            frameAddr: int
          ...
          for i = 0 to aPageTable.numberOfPages - 1
            ...
            -- example
            -- if frameAddr = 1032
            -- first page frame is 1000
            -- PAGE_SIZE = 8
            -- then we would get page 4 for the bitNumber
            -- we can then clear the bit showing that that page is free
            framesInUse.ClearBit(bitNumber)
          endFor

          numberFreeFrames = numberFreeFrames + aPageTable.numberOfPages -- we freed up this many pages, so adjust count accordingly
          newFramesAvailable.Signal(&frameManagerLock) -- we "hand-off" frames to next waiting thread
          frameManagerLock.Unlock()
        endMethod
...
-- All of the Handle functions are pretty straighforward since they only print things.
...
-----------------------------  Handle_Sys_Shutdown  ---------------------------------
  ...
  function Handle_Sys_Shutdown ()
      FatalError("Handle_Sys_Shutdown invoked!") -- use fatal error to simulate shutdown of the os
      nl()
      -- kind of a bad simulation, but it works. We just throw a fatalError instead of a "proper" shutdown.
...
-----------------------------  Handle_Sys_Exec  ---------------------------------
  ...
  function Handle_Sys_Exec (filename: ptr to array of char) returns int
      var
        ...
        initPC: int  -- PC intial instruction
        initUserStackTop: int
        initSystemStackTop: ptr to int
        oldIntStat: int
        newAddrSpace: AddrSpace
        ret: int
        fileBuffer: array [MAX_STRING_SIZE] of char 

      fileBuffer = new array of char { MAX_STRING_SIZE of '0' }
      ...
      -- from p5.pdf doc:      
      ret = currentThread.myProcess.addrSpace.GetStringFromVirtual(&fileBuffer, filename asInteger, MAX_STRING_SIZE)
      if ret < 0 
        return -1 -- Error copying  
      endIf

      file = fileManager.Open(&fileBuffer)
      if file == null -- could not load file for some reason
        return -1
      endIf
      
      initPC = file.LoadExecutable(&newAddrSpace)
      -- load executable bit from file, hopefully get a initial address for a program counter
      if initPC == -1
        return -1
      endIf

      fileManager.Close(file) -- done reading file
      frameManager.ReturnAllFrames(&currentThread.myProcess.addrSpace)
      currentThread.myProcess.addrSpace = newAddrSpace -- set currentprocess address space to newAddrSpace

      initUserStackTop = newAddrSpace.numberOfPages * PAGE_SIZE
      initSystemStackTop = &currentThread.systemStack[SYSTEM_STACK_SIZE-1]

      -- 1. disable interrupts because of 2. and 3. possible race conditions
      -- interrupts are enabled when thread is resumed for first time.
      oldIntStat = SetInterruptsTo(DISABLED) 
      -- 2. point cpu registers to the logical address space
      currentThread.myProcess.addrSpace.SetToThisPageTable ()
      -- 3. Become a user thread.
      currentThread.isUserThread = true
      -- 4. Set the system stack top, 5-9 are done inside BecomeUserThread
      BecomeUserThread(initUserStackTop, initPC, initSystemStackTop asInteger)
        
      return 3000 -- return some unique value
    endFunction

-----------------------------  Handle_Sys_Create  ---------------------------------

  function Handle_Sys_Create (filename: ptr to array of char) returns int
      -- ohh man these are going to be fun
      -- Edit: wasnt that bad
      -- just seemed bad, but its just like the others, only with the addition that you need
      -- to create a userspace buffer first to copy the kernel space buffer into before reading
      -- the contents of the buffer to ensure it is "safe" to read
      var
        ret: int
        fileBuffer: array [MAX_STRING_SIZE] of char
      ...
      ret = currentThread.myProcess.addrSpace.GetStringFromVirtual(&fileBuffer, filename asInteger, MAX_STRING_SIZE)
      if ret < 0
        -- GetSTringFromVirtual failed 
        return -1
      endIf

      print("Handle_Sys_Create invoked!")
      nl()
      print("filename virtual address = ")
...
-----------------------------  OpenFile  ---------------------------------
  ...
  function InitFirstProcess ()
      ...
      var firstProcess: ptr to Thread
      firstProcess = threadManager.GetANewThread()
      firstProcess.name = "theFirstProcess"
      firstProcess.Fork(StartUserProcess, 0) -- some arg because fork needs an arg, but we dont care
  ...
  function StartUserProcess (arg: int)
      ...
      pcb = processManager.GetANewProcess() -- get a pointer to the Process Control Block
      pcb.myThread = currentThread -- this should be the the first process thread
      currentThread.myProcess = pcb -- set current thraed parent process to pcd we just created
      -- this seems a bit backwards to me, to create a thread, and then allocate a process and give it to the process...
      file = fileManager.Open("TestProgram2")
      if file == null -- could not load file for some reason
        FatalError("Unable to load file for some reason.")
      endIf
      
      initPC = file.LoadExecutable(&pcb.addrSpace)
      -- load executable bit from file, hopefully get a initial address for a program counter
      if initPC == -1
        FatalError("Didn't get an initial PC, so exiting.")
      endIf
