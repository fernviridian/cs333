code Main
  ...
  -- OS Class: Project 2
  -- Codename: Harry Porter and the Sorcerer's Kernel
  --
  -- BEN REICHERT
  --
...
-- https://github.com/carrbs/p2/
-- https://github.com/ladinu/cs333
-- https://github.com/SYKim4131/Blitz-OS-Project
-- https://github.com/lewisric/Frogs-N-Feels/
-- https://github.com/JuShuck/CS333
-- https://github.com/locdle/PSU/
...
-----------------------------  ProducerConsumer  ---------------------------------
  ...
  var
    ...
    -- added a mutex lock and two semaphores for buffer counting
    mutex: Mutex = new Mutex -- the mutex lock
    full: Semaphore = new Semaphore -- semaphore to count how many slots are full
    empty: Semaphore = new Semaphore -- semaphore to count how many slots are empty
  ...
  function Producer (myId: int)
      ...
      for i = 1 to 5
        ...
        empty.Down() -- decrement empty counter, wait with semaphore
        -- make sure we use the semaphores for synchronization and then we 
        -- try to acquire the lock in that order. We then want to unlock the mutex
        -- and the use the opposite syncrhonization semaphore to signal/wait for a new event
        mutex.Lock() -- lock in critical section
        ...
        mutex.Unlock() -- unlock now that were done with critical section
        full.Up() -- increment full counter, signal with semaphore
      endFor
    endFunction
  ...
  function Consumer (myId: int)
      ...
      while true
        -- Perform synchroniztion...
        full.Down() -- decrement full counter, wait with semaphore
        ...
        -- Perform synchronization...
        mutex.Unlock() -- unlock mutex lock
        -- we want to acquire and release the mutex lock within the bounds of the semaphore
        -- so that we ensure that this section can only run by one thread at a time.
        empty.Up() -- increment empty counter, signal with semaphore for a waiting condition
        -- saying that we removed something from the full queue and therefore added one to the empty queue
...
-----------------------------  Dining Philosophers  ---------------------------------
  ...
  class ForkMonitor
    -- algorithm / method for fork monitor adapted from several resources listed at header
    superclass Object
    fields
      mutexLock: Mutex  -- mutex lock
      conditions: array[BUFFER_SIZE] of Condition
      status: array[BUFFER_SIZE] of int
    methods
      Init ()
      PickupForks (p: int)
      PutDownForks (p: int)
      PrintAllStatus ()
      getLeftPhilosopher (p: int) returns int
      getRightPhilosopher (p: int) returns int
      areNeighborsNotEating (p: int) returns bool
      testAndSetEating (p: int)
  endClass
  ...
  behavior ForkMonitor
    ...
    method Init ()
        ...
        var index: int
        status = new array of int{BUFFER_SIZE of THINKING} -- cool syntax in kpl for initializing vars without for loop so thats handy 
        mutexLock = new Mutex
        mutexLock.Init()
        conditions = new array of Condition{BUFFER_SIZE of new Condition}

        -- initialize the Condition types
        for index=0 to BUFFER_SIZE-1
          conditions[index].Init()
        endFor
      endMethod
    ...
    method PickupForks (p: int)
        ...
        -- the philosopher wants to wait so we can safely change their status to HUNGRY
        status[p] = HUNGRY
        -- need to print status since it changed
        self.PrintAllStatus()

        -- now we need to check if they can actually pickup forks they need to eat
        self.testAndSetEating(p)
        while(status[p] != EATING)
          conditions[p].Wait(&mutexLock)  -- wait until we can eat correctly, when we get signal from TASE
        endWhile
        -- if we recieve a condition Signal we know we can eat now from the TASE method
        -- when we get to this point we know we are EATING status
    ...
    method PutDownForks (p: int)
        -- we are done eating at this point so we want to give up our forks to neighbors
        --enter critical section
        mutexLock.Lock()

        status[p] = THINKING  -- the philosopher is thinking now since they are done eating and are not hungry yet
        -- since we changed status we need to print again
        self.PrintAllStatus()

        --printInt(self.getLeftPhilosopher(p))
        --print("\n")
        --printInt(self.getRightPhilosopher(p))
        --print("\n")
        self.testAndSetEating(self.getRightPhilosopher(p)) -- set the status of the philospher to the right ""
        self.testAndSetEating(self.getLeftPhilosopher(p)) -- set the status of the philosopher to the left to try to get him to eat
        --NOTE: weird that blitz outputs run the exact same way each time we compile
        -- where you would expect different threading outputs. when we recompile with little changes
        -- we can see that at output for that compilation stays the same, but is usually
        --different than the rest of the compilations. this results in weird outputs
        --where we would expect the philosophers to be equally eating two at a time
        --always, but more often then not the pattern is irregular and results in different
        --output where maybe only one eats at one time, then two eat, etc
        --self.PrintAllStatus()
    ...
    method testAndSetEating (p: int) 
        -- "Don't [T]est [A]nd [S]et [E]ating me bro!" 
        -- a test that gets run to see if we are hungry and our neighbors are not eating, so we can eat
        if (self.areNeighborsNotEating(p) && status[p] == HUNGRY)
          --print("printing status p: ")
          --printInt(p)
          -- print("\n")
          status[p] = EATING -- thats what were doing now
          self.PrintAllStatus() -- we change status so print it again
          conditions[p].Signal(&mutexLock) -- signal that we are now eating, caught in PickUpForks() Wait condition
        endIf
      endMethod

    method areNeighborsNotEating (p: int) returns bool
        -- returns true if both neighbors are not eating, false otherwise
        return (status[self.getRightPhilosopher(p)] != EATING && status[self.getLeftPhilosopher(p)] != EATING)
      endMethod

    method getRightPhilosopher (p: int) returns int
        --This method is used to get the id number of the philospher to the right in relation to p
        --URGHHH parentheses are very important as is order of operations
        --return (p + 4 % BUFFER_SIZE) is not the same! it leads to weird stack issues when running since P can be less than 0 in some cases
        return (p + 4) % BUFFER_SIZE
      endMethod

    method getLeftPhilosopher (p: int) returns int
          -- this method is used to get the id number of the philosopher to the left in relation to p
