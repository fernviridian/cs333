...
-----------------------------  Mutex  ---------------------------------
  ...
  behavior Mutex
    ...
    --         on the mutex.
      ...
      method Init ()
          -- we need to initialize variables to starting values and such
          heldBy = null
          -- initialize new list of threads 
          waitingThreads = new List [Thread]
          -- no waiting threads, duh we just started
      ...
      method Lock ()
          var interruptStatus: int
          -- disable interrupts while in lock method
          interruptStatus = SetInterruptsTo(DISABLED)
          -- critical section

          if heldBy == null
            heldBy = currentThread  -- if no one is holding the lock, currentThread grabs the lock now
          else
            waitingThreads.AddToEnd(currentThread) -- add current thread that wants the lock to the waiting list of threads that want the lock
            currentThread.Sleep()  -- put waiting thread to sleep so we dont waste cpu cycles
          endIf

          interruptStatus = SetInterruptsTo(interruptStatus)
        endMethod

      ----------  Mutex . IsHeldByCurrentThread  ----------

      method IsHeldByCurrentThread () returns bool
          -- pretty simple to follow, if heldby is current thread return true
          -- we could just do this in code, but with OOP its nicer to have a method to do it for us :)
      ...
      method Unlock ()
          ...
          if heldBy != currentThread
            FatalError("tried to unlock lock when we dont own it")
          endIf   

          nextThread = waitingThreads.Remove() -- get the next thread from threads that want the lock and give it the lock
          heldBy = nextThread
          -- make sure we actually have a thread and not null when we poped a thread off the waitingThreads list
          if nextThread != null
            nextThread.status = READY  -- next thread is prepped to run
            readyList.AddToEnd(nextThread) -- ready to run on readyList
          endIf
